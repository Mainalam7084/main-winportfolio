...
// Desktop icon (mouse-driven, hover effects)
function DesktopIcon({ id, name, Icon, color, selected, onSingleClick, onDoubleClick, onGlitchEnter, onGlitchLeave }) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (!isSel) { e.currentTarget.style.border = '2px solid #000'; e.currentTarget.style.background = 'rgba(250,204,21,0.25)' }
        onGlitchEnter?.()
      }}
      onMouseLeave={(e) => {
        if (!isSel) { e.currentTarget.style.border = '2px solid transparent'; e.currentTarget.style.background = 'transparent' }
        onGlitchLeave?.()
      }}
    >
...
function MobileDesktopIcon({ name, Icon, color, onTap }) {
  return (
    ...
    >
      <div style={{ width: 52, height: 52, color, flexShrink: 0 }}>
        <Icon size={52} />
      </div>
      <span style={{
        fontFamily: 'var(--font-family-pixel)',
...
export default function Desktop() {
  ...
  const handleSingleClick = useCallback((id) => setSelected(id), [])

  const openApp = useCallback((app, title, props = {}) => {
    openWindow({ app, title, props })
  }, [openWindow])

  const dockH = isMobile ? MOBILE_DOCK_H : 48
  ...
  return (
    ...
    >
        ...
        >
          {SYSTEM_SHORTCUTS.map((s) => (
            <MobileDesktopIcon
              ...
              Icon={s.Icon}
              color={s.color}
              onTap={() => openApp(s.app, s.name, s.path ? { initialPath: s.path } : {})}
            />
          ))}
          ...
          {fs.desktop.map((item) => (
            <MobileDesktopIcon
              ...
              onTap={() => {
                if (item.type === 'folder') openApp('explorer', item.name, { initialPath: item.name })
                else openApp('explorer', 'Desktop', { initialPath: 'Desktop' })
              }}
            />
          ))}
        </div>
      ) : (
        /* ── DESKTOP: column flow with hover effects ── */
        <div
          style={{
            position: 'relative', zIndex: 3,
      ...
      {/* OS label watermark */}
      <div style={{
        position: 'absolute', bottom: `${dockH + 8}px`, right: '12px', zIndex: 3,
